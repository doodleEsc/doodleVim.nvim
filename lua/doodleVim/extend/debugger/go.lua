...
go.setup = function(dap)
	...
	local ok, pkg = pcall(register.get_package, "delve")
	if not ok then
		vim.notify("Cannot find package delve.", vim.log.levels.ERROR)
		return
	end

	local adapter_bin_path = pkg:get_install_path() .. "/bin/dlv"
