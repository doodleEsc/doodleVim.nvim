...
python.setup = function(dap)
	...
	local ok, pkg = pcall(register.get_package, "debugpy")
	if not ok then
		vim.notify("Cannot find package debugpy.", vim.log.levels.ERROR)
		return
	end

	local adapter_bin_path = pkg:get_install_path() .. "/venv/bin/python"
